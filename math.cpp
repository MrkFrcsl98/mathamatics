...
int rational_root(const int *coeffs, int degree) {
    ...
    for (int p = 1; p <= a0; ++p) if (a0 % p == 0) {
        for (int q = 1; q <= an; ++q) if (an % q == 0) {
...
double matrix_determinant(double **mat, int n) {
    ...
    for (int p = 0; p < n; ++p) {
        ...
        for (int i = 0; i < n-1; ++i) free(submat[i]);
        free(submat);
    }
    return det;
}
...
int matrix_inverse(double **mat, double **inv, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (fabs(pivot) < 1e-12) { // Singular
            for (int x = 0; x < n; ++x) free(aug[x]);
            free(aug);
            return 0;
        }
        for (int j = 0; j < 2*n; ++j)
...
double power_iteration(double **A, int n, double *eigenvector, int max_iter, double tol) {
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        double diff = 0;
        for (int i = 0; i < n; ++i) diff += fabs(eigenvector[i] - b[i]);
        if (diff < tol) break;
    ...
    double lambda = 0;
    for (int i = 0; i < n; ++i) lambda += tmp[i] * eigenvector[i];
    free(b); free(tmp);
    return lambda;
}
...
// y^2 = x^3 + ax + b mod p
typedef struct { uint64_t x, y; int is_inf; } ECPoint;

ECPoint ec_add(ECPoint P, ECPoint Q, uint64_t a, uint64_t p) {
    if (P.is_inf) return Q;
    if (Q.is_inf) return P;
    if (P.x == Q.x && (P.y + Q.y) % p == 0) { ECPoint inf = {0,0,1}; return inf; }
    uint64_t m;
    if (P.x == Q.x && P.y == Q.y) { // doubling
...
// f: function pointer, a, b: interval, eps: tolerance, maxdepth: recursion limit
double adaptive_simpson(double (*f)(double), double a, double b, double eps, int maxdepth) {
    double c = (a + b) / 2;
    double fa = f(a), fb = f(b), fc = f(c);
    double simpson = (fa + 4*fc + fb) * (b - a) / 6;
    double recur(double l, double r, double fl, double fr, double fm, double approx, int depth) {
        double m = (l + r) / 2;
        ...
        return recur(l, m, fl, fm, fml, left, depth+1)
             + recur(m, r, fm, fr, fmr, right, depth+1);
    }
    return recur(a, b, fa, fb, fc, simpson, 0);
}
...
int solve_linear_system(double **A, double *b, double *x, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        int maxrow = i;
        for (int j = i+1; j < n; ++j)
            if (fabs(A[j][i]) > fabs(A[maxrow][i])) maxrow = j;
        if (fabs(A[maxrow][i]) < 1e-12) return 0;
        // Swap rows
        if (maxrow != i) {
...
int cubic_roots(double a, double b, double c, double d, double *roots) {
    if (fabs(a) < 1e-12) return quadratic_roots(b, c, d, roots, roots+1);
